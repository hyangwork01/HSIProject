...
def construct_model(robot_name: str, keypoint_names: Sequence[str]):
    ...
    if robot_name == "h1":
        humanoid_mjcf = mjcf.from_path("core/data/assets/mjcf/h1.xml")
    elif robot_name == "g1":
        humanoid_mjcf = mjcf.from_path("core/data/assets/mjcf/g1.xml")
    else:
...
def retarget_motion(motion: SkeletonMotion, robot_type: str, render: bool = False):
    ...
    if robot_type in ["h1", "g1"]:
        ...
        skeleton_tree = SkeletonTree.from_mjcf(
            f"core/data/assets/mjcf/{robot_type}.xml"
        )
