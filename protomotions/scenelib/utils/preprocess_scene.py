...
import numpy as np
from protomotions.scenelib.utils.usd_utils import get_homeassets_list,euler_to_quat
...
def preprocess_single_scene(single_scene_usd_path):
    ...
    for id,home_asset in enumerate(objects):
        ...
        obj = {
            "Id": id,
            "Objpath": obj_path,
            "Objname": home_asset["prim_category"],
            "Translate": obj_translate,
            "RotateXYZW": obj_rotateXYZW,
            "Scale": obj_scale,
        }
    ...
    for id, obj in enumerate(scene["objects"]):

        new_usd_path = os.path.join(os.path.dirname(single_scene_usd_path),obj["Objpath"])
        ...
        for prim in stage.Traverse():
            ...
            if has_hinge(stage, prim):
                scene["articulation_list"].append(obj["Id"])
                flag = True
        ...
        if not flag:
            for prim in stage.Traverse():
                ...
                if prim.HasAPI(UsdPhysics.RigidBodyAPI):
                    scene["rigidbody_list"].append(obj["Id"])
                    flag = True       
            if not flag:
                raise ValueError(f"ID:{obj['Id']} Objname:{obj['Objname']} 该物体没有rigidbody和articulation")
            
        if obj["Objname"] in filter_name_list:
            scene["filter_list"].append(obj["Id"])

    for id in scene["rigidbody_list"]:
        new_usd_path = os.path.join(os.path.dirname(single_scene_usd_path),obj["Objpath"])
        for prim in stage.Traverse():
...
def export_scene_json(usd_path):
                        ...
                        for usd_file in os.listdir(scene_path): # 遍历每个scene目录下的所有文件
                            if os.path.basename(usd_file) == "Instance.usda":
                                ...
                                scene = {
                                    ...
                                    "scene_type": name,
                                    "cfg_path": os.path.join(os.path.relpath(scene_path,usd_path),"config.json"), # 相对于scene根目录的相对路径[即home]
                                }
                                preprocess_single_scene(usd_file_path)
    ...
    json_path = os.path.join(usd_path, "config.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(scenes_json, f, ensure_ascii=False, indent=4)
    print(f"导出完成：{json_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="将 USD 场景物体相对变换导出为指定格式 JSON")
