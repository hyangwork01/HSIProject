...
from core.simulator.base_simulator.config import MarkerConfig, VisualizationMarker, MarkerState, SimulatorConfig
...
from core.envs.hoi.components.motion_manager import MotionManager

from core.utils.motion_lib import MotionLib
from core.envs.hoi.components.scenelib.scenelib import SceneLib

class BaseEnv:
    """
    这个类是可用于初始化smpl或者机器人的初始动作是默认动作还是根据motion_lib中的动作片段来截取一帧的动作初始化。hybrid是混合初始化。
    """    
    ...
    def __init__(self, config, device: torch.device, *args, **kwargs):
        ...
        """
        simulator初始化，从config中获取simulator的配置，然后根据配置创建simulator
        """
        ...
        self.create_termination_heights()
        ...
        """
        创建motion_lib，如果config中motion_lib是None，则state_init必须是Default，否则会报错，和参考动作相关代码
        """
        if self.config.motion_lib is not None and self.config.motion_lib.motion_file is not None:
            self.motion_lib: MotionLib = self.create_motion_lib()
    ...
    def create_terrain_and_scene_lib(self):
        ...
        self.terrain: SceneTerrain = TerrainClass(
    ...
    def create_termination_heights(self):
    ...
    def reset(self, env_ids=None):
        ...
        if len(env_ids) > 0:
            ...
            obj_new_states = self.sample_obj_location(new_states,env_ids)
            self.simulator.reset_objects(obj_new_states,env_ids)
