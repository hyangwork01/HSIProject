...
from core.envs.base_env.env_utils.humanoid_utils import (
...
from core.envs.base_env.components.humanoid_obs import HumanoidObs
from core.envs.base_env.components.terrain_obs import TerrainObs
...
class BaseEnv:
    ...
    """
    这个是用于create motion的管理器用于管理motion在每个环境中的使用
    """
    ...
    ###############################################################
    """
    这行代码应该是去先生成地图高度图所需要使用的角色脚底下的绿色小球，之后在其他代码处再把这些小球放到正确的位置上。
    """
    ...
    """
    设置终止的高度条件，甚至会专门去设置头部的高度条件[取头部高度和终止高度的最大值]
    """
    ...
    """
    利用config中的motion_lib获取要用于初始化的类，并实例化
    """
