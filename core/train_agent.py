...
from core.agents.ppo.agent import PPO  # noqa: E402
...
def main(config: OmegaConf):
    ...
    """
    读取对应配置文件后，转成dict，然后resolve为false可以保持配置文件中的占位符和${}符号不改变+不报错
    """
    ...
    """
    config会优先使用之前checkpoint对应的yaml文件，只有在不存在checkpoint文件时才会使用当前yaml文件
    """
    agent.fit()
