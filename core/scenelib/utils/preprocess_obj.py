...
import json
import argparse
from pxr import Usd, UsdGeom, Gf, UsdUtils,Sdf,PhysxSchema
...
def check_rigidbody(stage:Usd.Stage):
    ...
    for prim in stage.Traverse():
        # flag_rigid      = prim.HasAPI(UsdPhysics.RigidBodyAPI)       or prim.HasAPI(PhysxSchema.PhysxRigidBodyAPI)
        # flag_collision  = prim.HasAPI(UsdPhysics.CollisionAPI)       or prim.HasAPI(PhysxSchema.PhysxCollisionAPI)
        # flag_mass       = prim.HasAPI(UsdPhysics.MassAPI) 
        applied = prim.GetAppliedSchemas()
        for schema in applied:
            if schema =='PhysicsRigidBodyAPI':
                flag_rigid = True
            if schema == 'PhysicsCollisionAPI':
                flag_collision = True
            if schema == 'PhysicsMeshCollisionAPI':
                flag_mass = True

        # if prim.IsA(UsdPhysics.RigidBodyAPI):
        #     flag_rigid = True
        # if prim.IsA(PhysxSchema.PhysxRigidBodyAPI):
        #     flag_rigid = True
        # if prim.IsA(UsdPhysics.CollisionAPI):
        #     flag_collision = True
        # if prim.IsA(PhysxSchema.PhysxCollisionAPI):
        #     flag_collision = True
        # if prim.IsA(UsdPhysics.MassAPI):
        #     flag_mass = True
        # if prim.IsA(PhysxSchema.PhysxMassAPI):
        #     flag_mass = True
        # 如果三种都找到了，就没必要再继续遍历了
...
def preprocess_rigidbody(usd_path,output_path=None):
    ...
    # 处理usda自身坐标系的中心点问题,以及实现扁平化
    if output_path is not None:
        if output_path.endswith('.usda') or output_path.endswith('.usd'):
            cleanup_stage(usd_path,output_path)
        elif os.path.isdir(output_path):
            output_path = os.path.join(output_path,os.path.basename(usd_path))
            cleanup_stage(usd_path,output_path)
        else:
    ...
    stage = Usd.Stage.Open(output_path, load=Usd.Stage.LoadAll)
...
if __name__ == '__main__':
    ...
    for dirpath, dirnames, filenames in os.walk(object_dir):
        ...
        for filename in filenames:
            if filename == 'Instance.usda' or filename == 'Instance_flatten.usda':
                # 拼出完整路径
